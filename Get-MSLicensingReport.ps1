...
$PlanNameToCategory = @{
    ...
    "ADALLOM_S_O365"                    = "MDA"
    "ADALLOM_FOR_AATP"                  = "MDA"
    "ADALLOM_S_DISCOVERY"               = "MDA"
    ...
    "RMS_S_ENTERPRISE"                  = "Purview_AIP_P1"
    ...
    "PREMIUM_ENCRYPTION"                = "Purview_eDiscovery"
    ...
    "INFO_GOVERNANCE"                   = "Purview_DataLifecycle"
    "RECORDS_MANAGEMENT"                = "Purview_DataLifecycle"
    "INFORMATION_BARRIERS"              = "Purview_InfoBarriers"
    "CONTENT_EXPLORER"                  = "Purview_ContentExplorer"
    "ML_CLASSIFICATION"                 = "Purview_MLClassification"

    # --- Productividad ---
    "EXCHANGE_S_ENTERPRISE"             = "Exchange_Online"
    "EXCHANGE_S_STANDARD"               = "Exchange_Online"
    "EXCHANGE_S_FOUNDATION"             = "Exchange_Online"
...
$CategoryGroups = [ordered]@{
    ...
    "Purview (E5 Compliance)"         = @("Purview_DLP", "Purview_Audit", "Purview_eDiscovery", "Purview_InsiderRisk", "Purview_CommCompliance", "Purview_DataLifecycle")
...
Write-Host @"
  ...
  __  __ ____  _    _                    _
 |  \/  |___ \| |  (_) ___ ___ _ __  ___(_)_ __   __ _
 | |\/| | __) | |  | |/ __/ _ \ '_ \/ __| | '_ \ / _`` |
 | |  | |/ __/| |__| | (_|  __/ | | \__ \ | | | | (_| |
 |_|  |_|_____|____|_|\___\___|_| |_|___/_|_| |_|\__, |
                                                   |___/
  Microsoft 365 Licensing Assessment v2
...
try {
    ...
    Write-OK "Total de usuarios en el tenant: $TotalTenantUsers"
} catch {
...
foreach ($User in $AllUsers) {
    ...
    if ($UserSkuIds.Count -gt 1) {
        # Contar cuantos SKUs proveen cada categoria
        $CategoryProviders = @{}
        foreach ($Sid in $UserSkuIds) {
            if ($SkuCategories.ContainsKey($Sid)) {
                foreach ($Cat in $SkuCategories[$Sid]) {
    ...
    if ($UserDuplicates.Count -gt 0) {
        $WasteReasons.Add("Duplicate_License")
...
foreach ($Cat in $SecurityCategories) {
    ...
    foreach ($Sku in $SubscribedSkus) {
        ...
        if ($SkuCategories.ContainsKey($SkuId) -and ($SkuCategories[$SkuId] -contains $Cat)) {
            ...
            $SkuCount++
            $ProvidedBy.Add("$(Get-FriendlySkuName -SkuPartNumber $Sku.SkuPartNumber) ($($Sku.PrepaidUnits.Enabled))")
        }
    }
    ...
    # Clasificar — distinguir overlap de overlicensing real
    ...
    $Status = if ($UsersActive -gt $TotalSeats) { "UNDERLICENSED" }
              ...
              elseif ($PctUsed -lt 30) { "OVERLICENSED" }
...
Write-Host "  Usuarios con licencias duplicadas:    $WasteDuplicateLicenses" -ForegroundColor $(if ($WasteDuplicateLicenses -gt 0) { "Yellow" } else { "Green" })
