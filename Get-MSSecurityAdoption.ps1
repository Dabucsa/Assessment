    ...
    for ($retry = 1; $retry -le $MaxRetries; $retry++) {
        try {
            ...
            if ($retry -lt $MaxRetries) {
                # Leer Retry-After header si es throttling, sino backoff exponencial
                $Wait = 5 * [math]::Pow(2, $retry - 1)  # 5, 10, 20, 40...
...
function Invoke-KQL {
    ...
    $Result = Invoke-GraphSafe -Method POST -Uri "https://graph.microsoft.com/v1.0/security/runHuntingQuery" -Body $Body
...
Write-Host @"
 ...
 / ____| |        | | | | |      | |    | |           | | (_)
| (___ | |__   ___| | | | |_ __  | |_ __| | ___  _ __ | |_ _  ___  _ __
 \___ \| '_ \ / _ \ | | | | '_ \ | __/ _` |/ _ \| '_ \| __| |/ _ \| '_ \
 ____) | | | |  __/ | | | | |_) || || (_| | (_) | |_) | |_| | (_) | | | |
|_____/|_| |_|\___|_|_| |_| .__/  \__\__,_|\___/| .__/ \__|_|\___/|_| |_|
                           | |                   | |
                           |_|                   |_|
  Microsoft 365 Security Adoption Assessment v1
...
if ($HasAdvHunting) {
    ...
    # Verificar acceso a Advanced Hunting con una query generica (AlertInfo existe en todos)
    Write-Step "Verificando acceso a Advanced Hunting..."
    $KQLAvailable = $false
    try {
        $TestBody = @{ Query = "AlertInfo | take 1" } | ConvertTo-Json
        $TestResult = Invoke-GraphSafe -Method POST -Uri "https://graph.microsoft.com/v1.0/security/runHuntingQuery" -Body $TestBody
