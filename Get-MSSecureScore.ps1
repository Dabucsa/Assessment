    ...
    for ($retry = 1; $retry -le $MaxRetries; $retry++) {
        try {
            ...
            if ($retry -lt $MaxRetries) {
                $Wait = if ($ErrMsg -match "429|Throttl") { 30 } else { 5 }
...
Write-Host @"
  ...
  ____                            ____
 / ___|  ___  ___ _   _ _ __ ___/ ___|  ___ ___  _ __ ___
 \___ \ / _ \/ __| | | | '__/ _ \___ \ / __/ _ \| '__/ _ \
  ___) |  __/ (__| |_| | | |  __/___) | (_| (_) | | |  __/
 |____/ \___|\___|\__,_|_|  \___|____/ \___\___/|_|  \___|
...
$NextLink = "https://graph.microsoft.com/v1.0/security/secureScoreControlProfiles?`$top=100"
while ($NextLink) {
...
if ($AllControlProfiles.Count -gt 0) {
    foreach ($Ctrl in $AllControlProfiles) {
        $MaxCtrlScore = [double]$Ctrl.maxScore
